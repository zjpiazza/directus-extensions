...
export function useWorkflowTheme() {
  ...
  const getNodeColors = (theme: WorkflowTheme, options: NodeStyleOptions) => {
  ...
  const getNodeStyle = (options: NodeStyleOptions): ComputedNodeStyle => {
    ...
    const shape = colors.shape || { type: 'rectangle', borderRadius: '4px', borderStyle: 'solid', borderWidth: '1px' };

    const style: ComputedNodeStyle = {
      background: `linear-gradient(135deg, ${colors.primary}dd, ${colors.primary}aa)`,
      border: `${shape.borderWidth} ${shape.borderStyle} ${colors.border || 'rgba(255,255,255,0.2)'}`,
      color: colors.text || '#ffffff'
    };

    // Apply shape-specific border radius
    if (shape.borderRadius && shape.type !== 'diamond' && shape.type !== 'hexagon') {
      (style as any).borderRadius = shape.borderRadius;
    }
  ...
  const getEdgeStyle = (state: 'default' | 'selected' | 'hover' = 'default') => {
    const theme = currentTheme.value;
    if (!theme.edges) return {};
    
    return theme.edges[state] || theme.edges.default;
  };

  const getCanvasStyle = () => {
    const theme = currentTheme.value;
    if (!theme.canvas) return { background: 'var(--theme-background)' };
